...
export const AuthProvider = ({ children }) => {
   ...
   * Prevents session expiration during long admin sessions
   */
   ...
   * Prevents logout when switching tabs
   */
  const handleVisibilityChange = useCallback(() => {
    ...
    if (isVisible && isAuthenticated) {
      console.log('📱 Tab became active - validating session');
      // Validate session when returning to tab
      checkUserAuth();
    }
  }, [isAuthenticated]);

  // Check user auth and fetch user profile
  const checkUserAuth = useCallback(async (skipLoading = false) => {
    try {
      ...
      if (error instanceof Error && error.message === 'Auth check timeout') {
        console.warn('⏱️ Auth check timed out - proceeding with unauthenticated state');
        setUser(null);
        setIsAuthenticated(false);
        setUserRole(null);
        setIsAdmin(false);
      ...
      if (!skipLoading) setIsLoadingAuth(false);
      setAuthChecked(true);
    }
  }, []);
  ...
  // Supabase auto-refresh runs ~1 min before expiry, but manual refresh ensures it
  useEffect(() => {
    if (isAuthenticated) {
      // Initial refresh immediately after auth
      refreshAuthToken();
      
      // Set up periodic token refresh (every 5 minutes)
      tokenRefreshIntervalRef.current = setInterval(refreshAuthToken, 5 * 60 * 1000);
      console.log('🔄 Token refresh loop started (5-minute interval)');
    }

    return () => {
      if (tokenRefreshIntervalRef.current) {
        clearInterval(tokenRefreshIntervalRef.current);
        console.log('🛑 Token refresh loop stopped');
      }
    };
  ...
  const logout = async () => {
    try {
      // Stop token refresh loop before logout
      if (tokenRefreshIntervalRef.current) {
        clearInterval(tokenRefreshIntervalRef.current);
      }
