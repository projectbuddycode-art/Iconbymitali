...
import { Plus, Pencil, Trash2, Upload, FileText, Eye, EyeOff, AlertCircle } from "lucide-react";
...
export default function BlogTab() {
  ...
  // Autosave hook
  const { saveState, clearDraft } = useAutosaveDraft('blog-form', formData);
  
  // Beforeunload warning
  const hasUnsavedChanges = JSON.stringify(formData) !== JSON.stringify(editingPost ? {
    title: editingPost.title,
    excerpt: editingPost.excerpt || "",
    content: editingPost.content,
    image: editingPost.image || "",
    author: editingPost.author || "Mitali Dhumal",
  ...
  const savePost = async () => {
    try {
      ...
      setSaveError(error.message || "Error saving post");
    }
  };
